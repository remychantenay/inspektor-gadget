...
func WrapInErrInvalidParam(param string, err error) error {
	return fmt.Errorf("invalid parameter '%s': %w", param, err)
}

func WrapInErrParamsNotSupported(params string) error {
	return fmt.Errorf("parameters not supported: %s", params)
}
...
func WrapInErrInvalidArg(param string, err error) error {
