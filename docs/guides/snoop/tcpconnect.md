...
```bash
$ kubectl gadget snoop tcpconnect --podname mypod
```
...
use `kubectl gadget snoop tcpconnect --selector key=value` to filter the pods by
...
```bash
$ kubectl gadget snoop tcpconnect --podname mypod  # (still running in old terminal)
NODE             NAMESPACE        POD              CONTAINER       PID    COMM         IP SADDR            DADDR            DPORT
